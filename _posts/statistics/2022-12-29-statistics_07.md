...
[표본평균](/statistics/statistics_03/#표본평균)을 확률변수로 표현하면 아래와 같이 관측된 값에 자료 중 그 값이 차지하는 비율을 곱하여 더한 것으로 표현할 수 있다.  

$$\overline{x} = \sum_{i=1}^{n}x_{i} \frac{n_{i}}{n} = \sum_{i=1}^{n}x_{i}p_{i}$$

이 때 $$p_{i} = n_{i}/n$$ 이 값이 $$i$$인 표본의 비율이므로, [상대도수의 극한의 개념](/statistics/statistics_05/#3-통계적-확률)에 따라 $$n$$이 무한대로 발산하면 모집단에서 각 사건이 발생할 확률이 되어 이를 통해 모평균을 구할 수 있다. 이를 **[큰 수의 법칙(Law of large numbers, LLN)](https://en.wikipedia.org/wiki/Law_of_large_numbers)**이라 한다.  
...
이산확률변수 $$X$$의 기대값은 아래와 같이 계산할 수 있다.  

$$E(X) = \sum_{x}xf(x) = \mu$$

연속확률변수 $$X$$의 기대값은 아래와 같이 계산할 수 있다.  

$$E(X) = \int xf(x)dx = \mu$$

### 4-1. 변환된 확률변수의 기대값

확률변수 $$X$$의 변환된 확률변수를 $$Y = g(X)$$라 할 때, $$Y$$의 기대값은 아래와 같이 구할 수 있다.  
