...
related_posts:
    - _posts/softwareengineering/2022-08-11-sql_05.md
    - _posts/softwareengineering/2022-08-14-sql_07.md
...
- `CHR/CHAR(ASCII)`
    - ASCII 코드를 문자로 변경
- `CONCAT('str1', 'str2')`
    - 'str1'과 'str2'을 연결. [합성연산자](/softwareengineering/sql_05/#3-5-연산자)와 동일
...
❗ Oracle의 경우 데이터에 접근할 때 행 전체 칼럼을 메모리에 로드하기 때문에 `SELECT`절에 지정되지 않은 칼럼으로도 정렬이 가능하나, `FROM`절이 [서브쿼리](/softwareengineering/sql_09/#1-3-위치에-따른-서브쿼리-분류) 형태로 되어있을 경우(인라인 뷰)에는 불가능하다.  
