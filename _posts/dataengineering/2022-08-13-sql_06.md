...
|-|-|
|[문자형 함수](#2-1-문자형-함수)<br>[숫자형 함수](#2-2-숫자형-함수)<br>[날짜형 함수](#2-3-날짜형-함수)<br>[변환형 함수](#2-4-변환형-함수)<br>[NULL 관련 함수](#2-6-null-관련-함수)|[집계 함수(Aggregate Function)](#3-1-집계-함수)<br>[그룹 함수(Group Function)](/dataengineering/sql_10/#2-그룹-함수)<br>[윈도우 함수(Window Function)](/dataengineering/sql_10/#3-윈도우-함수)|
{:.scroll-table}
...
Oracle의 경우 데이터에 접근할 때 행 전체 칼럼을 메모리에 로드하기 때문에 `SELECT`절에 지정되지 않은 칼럼으로도 정렬이 가능하나, `FROM`절이 [서브쿼리](/dataengineering/sql_09/#1-3-위치에-따른-서브쿼리-분류) 형태로 되어있을 경우(인라인 뷰)에는 불가능하다는 특징이 있다.  
