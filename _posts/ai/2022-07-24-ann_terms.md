...
인공신경망에서 손실 함수의 최소값을 찾기 위해, 즉 문제 해결에 최적화된 가중치들을 찾아내기 위해 경사하강법을 사용한다. 이 때 계산의 효율을 높이기 위해 일부 데이터는 건너 뛰면서 계산을 수행하게 되는데, 이 과정에서 어떤 지점에서 계산을 수행할지를 결정하는 함수를 **최적화 함수(Optimizer)**라고 한다. 즉, 인공신경망의 현재의 손실 함수 값이 감소하는 방향으로 파라미터를 업데이트 시킬 때 가장 중요한 어느 방향으로 얼만큼 갈 것인지 결정하는 함수를 말한다.  
