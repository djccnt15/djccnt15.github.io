...
임시 칼럼을 만들어서 조건에 따라 값을 넣어준 다음에 임시 칼럼을 지운다.  
...
데이터 타입을 원본 데이터와 같도록 수정하고, 임시 칼럼을 지운다.  
...
## 3. ❗numpy.select 사용
내가 가장 선호하는 방식으로, 하나의 조건에 따른 `True/False` 결과만을 연산하는 앞선 두 방법들과 달리 조건의 리스트를 무한정 늘릴 수 있고, 연산 순서는 고려하지 않아도 되지만 **조건의 순서를 고려해야 한다.**  
