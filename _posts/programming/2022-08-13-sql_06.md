...
- `GROUP BY`절을 통해 소그룹 별 기준을 정한 후, `SELECT`절에 집계 함수 사용
...
`ORDER BY`절은 SQL 문장의 제일 마지막에 위치하여 데이터를 특정 칼럼을 기준으로 정렬하여 출력하는데 사용한다. 정렬 기준을 `ALIAS`명이나 칼럼 순서를 나타내는 정수로도 표시 가능하며, 순서대로 적용할 여러 개의 정렬 기준을 지정하는 것도 가능하다.  
...
SQL에서 SELECT 문장의 실행 순서는 다음과 같으며, 옵티마이저가 SQL 문장의 `SYNTAX`, `SEMANTIC` 에러를 점검하는 순서 역시 동일하다.  
