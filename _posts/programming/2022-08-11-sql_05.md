...
```sql
CREATE TABLE [TB_NAME] ([COL_NAME_1] [DATATYPE] [DEFAULT_VALUE], [COL_NAME_2] [DATATYPE] [DEFAULT_VALUE]);
```
...
```sql
-- Oracle
ALTER TABLE [TB_NAME] MODIFY ([COL_NAME_1] [DATATYPE] [DEFAULT_VALUE] NOT NULL, [COL_NAME_2] [DATATYPE] …);

-- MSSQL
ALTER TABLE [TB_NAME] ALTER ([COL_NAME_1] [DATATYPE] [DEFAULT_VALUE] NOT NULL, [COL_NAME_2] [DATATYPE] …);
```
...
```sql
ALTER TABLE [TB_NAME] ADD CONSTRAINT [CON_NAME] [CON_TYPE] (COL_LIST);
```
...
```sql
-- returns all row
SELECT (COL_LIST) AS [ALIAS] FROM [TB_NAME];

-- return unique values
SELECT DISTINCT [COL_NAME] AS [ALIAS] FROM [TB_NAME];
```
...
```sql
-- Oracle
SELECT [COL_NAME] || 'CHAR' FROM [TB_NAME];
