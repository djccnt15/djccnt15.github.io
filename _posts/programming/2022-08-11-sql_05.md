...
```sql
CREATE TABLE table_name (col_1 datatype dafault_value, col_2 datatype dafault_value);
```
...
```sql
-- Oracle
ALTER TABLE table_name MODIFY (col_1 datatype dafault_value NOT NULL, col_2 datatype …);

-- MSSQL
ALTER TABLE table_name ALTER (col_1 datatype dafault_value NOT NULL, col_2 datatype …);
```
...
**트랜잭션(Transaction)**은 데이터베이스의 논리적 연산단위를 말하며, 분리될 수 없는 한 개 이상의 데이터베이스 조작을 가리킨다. 트랜잭션의 특성은 다음과 같다.  
...
❗ Oracle의 경우 DBMS가 트랜잭션을 내부적으로 실행하며 사용자가 `COMMIT` 혹은 `ROLLBACK`을 수행해 주어야 트랜잭션이 종료되지만, MSSQL의 경우 기본적으로 AUTO COMMIT 모드이기 때문에 `COMMIT`과 `ROLLBACK`을 사용하려면 암시적/명시적 트랜잭션을 설정해야 한다.  
{:.note title='attention'}
...
```sql
CREATE CREATE LOGIN ID WITH PASSWORD='password', DEFAULT_DATABASE='database_name';
```
