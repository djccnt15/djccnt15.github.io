...
`JobExecution`은 `Job`의 단일 실행 시도를 뜻하며, 아래와 같은 배치 작업에 대한 다양한 정보로 구성된다.  
...
`Step`은 배치 작업의 독립적이고 순차적인 단계를 캡슐화한 도메인으로, 하나의 `Job`은 한 개 이상의 `Step`으로 구성된다.  

입력 자원의 설정 및 실질적인 배치 작업 로직에 대한 모든 정보를 갖고 있는 도메인으로, `Step`의 구현 방식은 크게 두 가지다.  

- **Chunk-oriented Processing**: 데이터를 읽고(Read) → 처리하고(Process) → 쓰는(Write) 과정을 일정 단위(chunk)로 묶어 트랜잭션을 처리하는 방식
- **Tasklet**: 단순한 작업을 하나의 메서드로 처리하는 방식으로, 파일 이동이나 프로시저 호출처럼 read-process-write 구조가 필요 없는 경우 활용

Chunk-oriented Processing의 동작 구조는 아래와 같다.  
...
`StepExecution`은 `Step`의 단일 실행 시도를 뜻하며, 아래와 같은 배치 작업에 대한 다양한 정보로 구성된다.  
...
key-value 구조로 이루어져 `Job`과 `Step`의 상태 정보를 DB에 저장하는 객체로, Batch의 세션 역할로 활용된다.  
...
`JobRepository`는 Spring Batch가 활용하는 객체들의 정보를 저장하는 DB 관련 메커니즘으로, `JobLauncher`, `Job`, `Step`의 구현체들에 대한 CRUD 기능을 제공한다.  
...
배치의 비즈니스 로직을 처리하는 도메인으로, 사용자는 `ItemProcessor` 인터페이스를 사용해 배치를 처리하는 로직을 주입할 수 있다. `ItemProcessor`는 선택사항(optional)으로, 변환이나 필터링 로직이 불필요한 경우 생략 가능하다.  
...
!!! note
    ...
    %JAVA_PATH% -Xms%XMS% -Xmx%XMX% -jar "%CLASSPATH%" --spring.config.location=%CONFIG_PATH%
    ```
