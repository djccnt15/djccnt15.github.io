...
!!! info
    참고로 하위 $\alpha$%에 해당하는 값을 $\alpha$ 백분위수(percentile)라고 하며, $p = \alpha / 100$일 때, $p$ [분위수(quantile)](#2-2)이라고 말한다.  
...
- 25% 지점: 제1사분위수($Q_{1}$)
- 50% 지점: 제2사분위수($Q_{2}$), [표본중앙값](#1-2)과 동일
- 75% 지점: 제3사분위수($Q_{3}$)
...
- $L_{1}(a)$의 경우 [표본중앙값](#1-2)($a = \widetilde{x}$)에서 최소화된다.
- $L_{2}(a)$의 경우 [표본평균](#1-1)($a = \overline{x}$)에서 최소화되며, $L_{2}(a)$를 $a$에 대해 미분한 미분계수가 0이 되는$$a$ 값이 거리의 합이 최소화 되는 지점이기 때문에 아래와 같이 유도할 수 있다.
